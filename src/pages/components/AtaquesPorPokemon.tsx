...
function ListaAtaquesPokemon(): JSX.Element {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const resultado = await axios.get("http://localhost:8000/api/pokemones");
  ...
  return (
    <Container className="position-relative">
     <h2 className="text-center mt-2 font-weight-bold">
      ...
      <Paginacion
      elementosTotatales={ataquesPokemon.length} elementosPorPagina={elementosPorPagina} onPageChange={paginacion} paginaActual={paginaActual}/>
      </div>
    </Container>
  );
}
